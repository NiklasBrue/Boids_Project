...
class Predator:
    def __init__(self,
        ...
        self.bounds = bounds
        # print("# DEBUG: predator, bounds: ", bounds)
    ...
    def draw(self, show_velocity=False, show_view=False, show_vectors=False):
        ...
        # print("# DEBUG: self.position, draw predator: ", self.position)
    ...
    def determine_nearest_boid(self, all_boids):
        ...
        for boid in all_boids:
            ...
            if (vector.magnitude(*diff) <= _PREDATOR_RANGE/self.scale_domain and
                    vector.angle_between(self.velocity, diff) <= _PREDATOR_VIEW_ANGLE):
    ...
    def determine_nearby_predators(self, all_predators):
        ...
        for pred in all_predators:
            ...
            diff = (relevant_position[0] - self.position[0],\
                    relevant_position[1] - self.position[1])
            if (pred != self and
                    vector.magnitude(*diff) <= _PREDATOR_RANGE/self.scale_domain and
                    vector.angle_between(self.velocity, diff) <= _PREDATOR_VIEW_ANGLE):
                yield pred
        return
    ...
    def attack(self, nearest_boid):
        ...
        if not nearest_boid:
            ...
            # print("# DEBUG: relevant_position (attack): ", relevant_position)
            diff = [relevant_position[0] - self.position[0],\
                    relevant_position[1] - self.position[1]]
    ...
    def update(self, dt, all_predators, all_boids, obstacles):
        # print("# DEBUG: new update predator")
        # print("# DEBUG: position before update: ", self.position)
        ...
        for i in range(0, len(self.position)):
            ...
            if self.position[i] >= self.wrap_bounds[i]:
                ...
                self.position[i] = self.position[i] + self.wrap_bounds[i] + _BOUNDARY_SLOP
        # print("# DEBUG: position after update: ", self.position)
