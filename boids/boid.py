...
class Boid:
    def __init__(self,
        ...
        self.bounds = bounds
        self.velocity = velocity/scale_domain
        self.scale_domain = scale_domain
        self.size = size/scale_domain
        self.color = color
        self.wrap_bounds = [i for i in bounds]
        self.change_vectors = []
        self.age = age
        self.viewing_range, self.viewing_angle, self.max_speed = self.characteristics_based_on_age(self.age)

    def characteristics_based_on_age(self, age):
        viewing_range = _BOID_RANGE-_BOID_RANGE*(age-0.5)**2
        viewing_angle = _BOID_VIEW_ANGLE-_BOID_VIEW_ANGLE*(age-0.5)**2
        max_speed = _MAX_SPEED-4*(age-0.5)**2
        return viewing_range, viewing_angle, max_speed

    def __repr__(self):
        return "Boid: position={}, velocity={}, age={}".format(
            self.position, self.velocity, self.age)
    ...
    def determine_nearby_boids(self, all_boids):
        ...
        for boid in all_boids:
            ...
            if (boid != self and
                    vector.magnitude(*diff) <= self.viewing_range/self.scale_domain and
                    vector.angle_between(self.velocity, diff) <= self.viewing_angle):
                yield boid
        return
    ...
    def determine_nearby_objects(self, all_objects, collision_distance):
        ...
        for obj in all_objects:
            ...
            diff = (relevant_position[0] - self.position[0],\
                    relevant_position[1] - self.position[1])
            if (obj != self and
                    vector.magnitude(*diff) <= collision_distance/self.scale_domain):
                yield obj
        return
    ...
    def update(self, dt, all_boids, attractors, obstacles, predators):
        ...
        predator_avoidance_vector = self.avoid_collisions(predators, _PREDATOR_COLLISION_DISTANCE)
        ...
        for factor, vec in self.change_vectors:
            self.velocity[0] += factor *vec[0]/self.scale_domain
            self.velocity[1] += factor *vec[1]/self.scale_domain

        # ensure that the boid's velocity is <= self.max_speed
        self.velocity = vector.limit_magnitude(self.velocity,\
                                                self.max_speed/self.scale_domain,\
                                                _MIN_SPEED/self.scale_domain)
